...
const genRef=()=>'NBG-'+Date.now().toString(36).toUpperCase().slice(-6)+'-'+Math.floor(Math.random()*9000+1000);
...
function startPipeline(){
  ...
  console.log('[NexusAI-BankGuard] Request payload', payload);
  ...
  .then(data => {
    console.log('[NexusAI-BankGuard] API response', data);
    apiData = data;
    log('Backend analysis payload received');
  })
  .catch(err => {
    console.error('[NexusAI-BankGuard] API error', err);
...
function checkAndFinish(){
  ...
  if (apiError) {
    clearInterval(eTimer);
    console.warn('[NexusAI-BankGuard] Stopped pipeline due to backend error');
    return;
  }
...
function finishWithData(data){
  ...
  console.log('[NexusAI-BankGuard] Final decision rendered:', S.decision);
...
function dlReceipt(){
  ...
  const h=`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>NexusAI-NexusAI-BankGuard Sanction</title>
...
<h1>⬡ NexusAI-BankGuard</h1><div class="sub">Neural Credit System · Official Sanction Document</div>
...
<div class="footer">NEXUSAI-NEXUSAI-BANKGUARD NEURAL CREDIT SYSTEM · ${S.ref}<br/>SYSTEM-GENERATED · RBI FAIR PRACTICES CODE COMPLIANT<br/>FULL COMPLIANCE AUDIT TRAIL ATTACHED</div>
</body></html>`;
  const blob=new Blob([h],{type:'text/html'});
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);
  a.download='NexusAI-NexusAI-BankGuard_Sanction_'+S.ref+'.html';a.click();
  toast('SANCTION DOCUMENT DOWNLOADED');
}

function dlAudit(){
  const lines=S.audit.map(l=>'['+l.t+'] '+l.m).join('\n');
  const hdr='NEXUSAI-NEXUSAI-BANKGUARD NEURAL CREDIT SYSTEM\nCOMPLIANCE AUDIT LOG\nAPPLICATION: '+S.ref+'\nGENERATED: '+new Date().toISOString()+'\n'+'─'.repeat(60)+'\n\n';
  const blob=new Blob([hdr+lines],{type:'text/plain'});
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);
  a.download='NexusAI-NexusAI-BankGuard_AuditLog_'+S.ref+'.txt';a.click();
    ...
    <div class="logo-wrap">
      <div class="logo-hex">
        <svg viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <text x="19" y="23" text-anchor="middle" font-family="Orbitron,sans-serif" font-size="9" fill="#00F5FF" font-weight="700">NBG</text>
        </svg>
      </div>
      <div>
        <div class="logo-name">NexusAI-BankGuard</div>
    ...
    <div class="receipt-panel">
      ...
      <div class="receipt-body">
        <div class="receipt-hdr">
          <div>
            <div class="receipt-logo-txt">⬡ NEXUSAI-BANKGUARD</div>
        ...
        <div class="receipt-footer">
          ...
          <div class="receipt-legal">NEXUSAI-NEXUSAI-BANKGUARD NEURAL CREDIT SYSTEM<br/>RBI FAIR PRACTICES CODE COMPLIANT<br/>${S.ref}</div>
        </div>
      </div>
    </div>
...
function checkBackendConnection() {
  fetch(`${BACKEND_URL}/health`)
    ...
    .then(data => {
      ...
      render();
      console.log('[NexusAI-BankGuard] Frontend is currently CONNECTED to the backend at ' + BACKEND_URL);
    })
    .catch(err => {
      console.error('Connection check failed:', err);
      isConnected = false;
      render();
      console.warn('[NexusAI-BankGuard] Frontend is currently running STANDALONE (Backend disconnected at ' + BACKEND_URL + ')');
    });
}
